            ...
            <View className="p-5 rounded-xl" style={{ backgroundColor: "#72063c50" }}>
                ...
                <View className="flex items-center justify-around mt-4 flex-row">
                    <View className=" rounded-xl overflow-hidden">
                        <Pressable android_ripple={{ color: "white" }} onPress={() => { if (screen < guessed) { setmax(parseInt(guessed)); setGuess(Math.floor((Math.random() * (parseInt(guessed) - min)) + min)) } }} className="bg-black px-10 py-2">
                            <Text className="text-white font-bold text-2xl">+</Text>
                        </Pressable>
                    </View>
                    <View className=" rounded-xl overflow-hidden">
                        <Pressable android_ripple={{ color: "white" }} onPress={() => { if (screen > guessed) { setmin(parseInt(guessed)); setGuess(Math.floor((Math.random() * (max - parseInt(guessed))) + parseInt(guessed))) } }} className="bg-black px-10 py-2 ">
                            <Text className="text-white font-bold text-2xl">-</Text>
                        </Pressable>
                    </View>
                </View>
